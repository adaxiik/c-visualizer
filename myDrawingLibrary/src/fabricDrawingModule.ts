...
class StackframeSlotWidget implements Widget {
    ...
    constructor(objectToDraw: DataModelStructures.Variable | DataModelStructures.StackFrame | DataModelStructures.HeapVariable, drawToCanvas: CustomCanvas, setStartPosX = 10, setStartPosY = 10, setConfig: StackframeSlotWidgetConfig) {
        this.canvas = drawToCanvas;
...
class HeapWidget implements Widget {
    canvas: CustomCanvas;
    ...
    constructor(heapToDraw: DataModelStructures.Heap, drawToCanvas: CustomCanvas, setStartPosX = 10, setStartPosY = 10, setConfig: HeapWidgetConfig) {
        this.canvas = drawToCanvas;
...
class ProgramStackWidget implements Widget {
    ...
    drawArrowsToHeap() {
        ...
        function drawArrowBetweenVariables(canvasToDrawTo: CustomCanvas, fromVariableObject : fabric.Object, toVariableObject : fabric.Object)
        {
...
export class FabricDrawingModule {
                            ...
                            if(drawingModuleThis.cache.pointers != undefined)
                            {
                                if(hoveredOverWidget.dataModelObject instanceof DataModelStructures.Variable && hoveredOverWidget.dataModelObject.isPointer)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if(toVariableObject instanceof fabric.Group && programStackWidget != undefined)
                                        {
    ...
    isWidgetOnHeap(widgetToCheck: Widget) : boolean {
        let cache = this.cache;

        function getHeapWidget() : HeapWidget | undefined {
            if(cache != undefined)
            {
                ...
                if(programStackWidgetsChildren != undefined)                        //Going through the program stack's children
                {
                    for(let i = 0; i < programStackWidgetsChildren.length; i++)
                    {
                        ...
                        if(currentChild instanceof HeapWidget)                      //If we found the heap widget
                            return currentChild;
                    }           
                }
            }
        ...
        function widgetInWidget(startSearchFrom: Widget, searchedForWidget: Widget) : boolean {
            //Checking the main widget for a match
